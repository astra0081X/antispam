...
local WARNS_BEFORE_KICK = 3     -- Number of warnings before mute
...
local function kick_player(name)
    local ok = minetest.kick_player(name)
    
    if ok then
        minetest.log("action", string.format("[Antispam] Player %s kicked.", name))
    else
        minetest.log("action", string.format("[Antispam] Failed to kick player %s", name))
    end
...
minetest.register_on_chat_message(function(name, message)
    ...
    if time_since_last < MIN_TIME_BETWEEN_MESSAGES_USEC then
        ...
        if player.message_count >= MESSAGES_BEFORE_WARN then
            ...
            if player.warning_count >= WARNS_BEFORE_KICK then
                kick_player(name)
                return true
            end
            
            minetest.chat_send_player(name, COLORS.WARNING .. 
                string.format("[AntiSpam] Warning [%d/%d]: Please slow down! Wait %d seconds between messages.", 
                    player.warning_count, WARNS_BEFORE_KICK, MIN_TIME_BETWEEN_MESSAGES))
        end
    else
        ...
        if msg_data.count >= MESSAGES_BEFORE_WARN then
            ...
            minetest.chat_send_player(name, COLORS.WARNING .. 
                string.format("[AntiSpam] Warning [%d/%d]: Please avoid repeating the same message. Wait %d seconds.", 
                    player.warning_count, WARNS_BEFORE_KICK, MESSAGE_RESET_TIME))
        end
    else
    ...
    func = function(name, param)
        ...
        if not option or not value then
            local help = {
                ...
                COLORS.TEXT .. "• warn: " .. COLORS.VALUE .. "Messages before warning " ..
                    COLORS.TEXT .. "(default: " .. COLORS.VALUE .. "6" .. COLORS.TEXT .. ")",
                COLORS.TEXT .. "• kick: " .. COLORS.VALUE .. "Warnings before kick " ..
        ...
        local options = {
            ...
            kick = {
                ...
                name = "Warnings before kick",
                desc = "warnings before kick"
            },
            reset = {
                update = function(v) 
